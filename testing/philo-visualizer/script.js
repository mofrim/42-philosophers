function parseLogs(logs) {
	...
	for (const line of lines) {
		...
		if (timestamp < previousTimestamp) {
			console.log(`Timestamp diff = ${previousTimestamp - timestamp}`);
...
for (const key in lastActions) {
		lastActions[key].end = maxTimestamp + averageDuration;
		console.log(`lastActions[key].end: ${lastActions[key].end}`);
...
function generateGantt(actions) {
	...
	philosophers.forEach(philosopher => {
		...
		label.textContent = `Philo ${philosopher}`;
		...
		actionsForPhilosopher.forEach(action => {
			...
			if (parseFloat(bar.style.width) > 3) {
				...
				actionText.textContent = `${Math.floor(duration)}ms`;
